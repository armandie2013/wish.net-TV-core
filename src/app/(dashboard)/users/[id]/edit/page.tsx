...
import { getAllLocations } from "@/services/location.service";
import { getAllPlans } from "@/services/plan.service";
import { TOKEN_EXPIRES_IN_OPTIONS } from "@/validations/user.validation";
...
export default async function EditUserPage({
  ...
  try {
    ...
    redirect("/users?error=usuario-no-encontrado");
  }
      ...
      <div className="overflow-hidden rounded-lg border border-slate-300 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900/60">
        <div className="border-b border-slate-200 px-3 py-3 dark:border-slate-800">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-1 max-w-2xl text-[12px] leading-snug text-slate-500 dark:text-slate-400">
                Modificá datos, rol, estado, plan, conexiones y duración del
                token de la cuenta.
              </p>
            </div>
        ...
        >
          <div className="grid gap-3 xl:grid-cols-[minmax(0,1fr)_320px]">
              ...
              <Panel title="Permisos y estado">
                <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                  <SelectField
                    ...
                    helper="Admin: panel completo. Operador: operación. Cliente: app TV."
                  />
                  ...
                  <Field
                    ...
                    required
                    helper="Cantidad de dispositivos simultáneos."
                  />

                  <SelectField
                    label="Duración token"
                    name="tokenExpiresIn"
                    defaultValue={user.tokenExpiresIn || "30d"}
                    options={TOKEN_EXPIRES_IN_OPTIONS.map((value) => ({
                      value,
                      label: TOKEN_EXPIRES_IN_LABELS[value] || value,
                    }))}
                    helper="El cambio aplica al próximo login."
                  />
                </div>
              </Panel>

              <Panel title="Localidad y plan">
                <div className="grid gap-3 md:grid-cols-2">
                  <SelectField
                    label="Localidad"
                    name="localidadId"
                    ...
                    options={[
                      { value: "", label: "Sin localidad específica" },
                    ...
                    helper="Si seleccionás una localidad, se usa como localidad principal del usuario."
                  />
                  ...
                  <SelectField
                    ...
                    helper="Para la app TV, el usuario necesita un plan con canales."
                  />
                </div>
            ...
            <Panel title="Resumen">
              <div className="space-y-3 rounded-lg border border-slate-200 bg-slate-50 px-3 py-3 dark:border-slate-800 dark:bg-slate-950/30">
                <div>
                  <p className="text-[10px] font-medium uppercase tracking-[0.12em] text-slate-500 dark:text-slate-400">
                    Cuenta
                  </p>
                ...
                <div className="grid grid-cols-2 gap-2 border-t border-slate-200 pt-3 dark:border-slate-800">
                  <SummaryItem label="Rol" value={user.rol} />
                  <SummaryItem label="Estado" value={user.estado} />
                  <SummaryItem
                    label="Conexiones"
                    value={String(user.conexionesPermitidas || 1)}
                  />
                  <SummaryItem
                    label="Token"
                    value={
                ...
                {user.mustChangePassword ? (
                  <div className="rounded-lg border border-amber-300 bg-amber-50 px-3 py-2 text-[11px] leading-snug text-amber-800 dark:border-amber-500/20 dark:bg-amber-500/10 dark:text-amber-200">
                    Este usuario debe cambiar su contraseña en el próximo
                    ingreso.
                  </div>
                ) : null}

                <div className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-[11px] leading-snug text-slate-500 dark:border-slate-800 dark:bg-slate-900/60 dark:text-slate-400">
                  La duración del token se aplica cuando el usuario vuelve a
                  iniciar sesión. Los tokens ya emitidos mantienen su
...
function SummaryItem({
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white px-2 py-2 dark:border-slate-800 dark:bg-slate-900/60">
      <p className="text-[10px] font-medium uppercase tracking-[0.1em] text-slate-500 dark:text-slate-500">
        {label}
      </p>

      <p className="mt-1 truncate text-[12px] font-medium text-slate-900 dark:text-white">
        {value}
      </p>
    </div>
  );
